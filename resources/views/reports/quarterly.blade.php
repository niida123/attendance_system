...
@section('title', 'Quarterly Attendance Report')

@section('content_header')
    <div class="d-flex align-items-center justify-content-between">
        <div>
            <h1 class="m-0 font-weight-bold" style="color:#1a1f36;font-size:1.4rem;letter-spacing:-0.3px;">
                <i class="fas fa-calendar-week mr-2" style="color:#4f46e5;"></i> Quarterly Attendance Report
            </h1>
            <ol class="breadcrumb mt-1 mb-0" style="background:transparent;padding:0;font-size:.8rem;">
                ...
                <li class="breadcrumb-item active" style="color:#6b7280;">Quarterly Attendance</li>
            </ol>
        </div>
    </div>
@stop
            ...
            <div class="card-header d-flex align-items-center justify-content-between flex-wrap" style="background:#fff;border-bottom:1px solid #f0f0f5;padding:18px 24px;gap:10px;">
                <div class="d-flex align-items-center">
                    <div style="width:38px;height:38px;border-radius:10px;background:linear-gradient(135deg,#4f46e5,#7c3aed);display:flex;align-items:center;justify-content:center;margin-right:12px;">
                        <i class="fas fa-calendar-week" style="color:#fff;font-size:.85rem;"></i>
                    </div>
                    <div>
                        <h3 class="mb-0 font-weight-bold" style="font-size:1rem;color:#1a1f36;">Quarterly Attendance</h3>
                        <small style="color:#9ca3af;font-size:.75rem;">Per-employee attendance totals for a selected quarter.</small>
                    </div>
                </div>
            ...
            <div class="card-body" style="padding:24px;background:#fafbff;">
                ...
                <div class="d-flex align-items-center flex-wrap mb-4" style="gap:10px;">
                    ...
                    <div style="width:170px;">
                        <select id="filterQuarter" class="form-control form-control-sm select2" style="width:100%;">
...
@section('js')
    ...
    $(document).ready(function () {
        ...
        $('#filterQuarter').val(Math.ceil((new Date().getMonth() + 1) / 3));

        $('.select2').select2({ theme: 'bootstrap4', width: '100%' });

        const DATA_URL  = '{{ route("reports.quarterly.data") }}';
        const PDF_URL   = '{{ route("reports.quarterly.export.pdf") }}';
        const EXCEL_URL = '{{ route("reports.quarterly.export.excel") }}';
        ...
        $('#filterYear, #filterQuarter, #filterOffice, #filterDepartment').on('change', () => table.ajax.reload());
        ...
        $('#btnReset').on('click', function () {
            ...
            $('#filterQuarter').val(Math.ceil((new Date().getMonth() + 1) / 3)).trigger('change');

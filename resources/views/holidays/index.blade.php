...
@section('title', 'Holidays - Attendance System')

@section('content_header')
    <div class="d-flex align-items-center justify-content-between">
        <div>
            <h1 class="m-0 font-weight-bold" style="color:#1a1f36;font-size:1.4rem;letter-spacing:-0.3px;">
                <i class="fas fa-calendar-alt mr-2" style="color:#4f46e5;"></i> Holidays
            </h1>
            <ol class="breadcrumb mt-1 mb-0" style="background:transparent;padding:0;font-size:.8rem;">
                ...
                <li class="breadcrumb-item active" style="color:#6b7280;">Holidays</li>
            </ol>
        </div>
    </div>
@stop
...
@section('content')
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <small style="color:#9ca3af;font-size:.75rem;">
                                Manage company holidays, set paid/unpaid status, and keep track of important dates.
                            </small>
                        </div>
                    </div>
                    <!-- Right Side -->
                        ...
                        <div class="col-md-6 mb-3 mb-md-0">
                            <div
                                ...
                                <div>
                                    ...
                                    <div style="font-size:.75rem;color:#9ca3af;margin-top:2px;">Total Holidays</div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6 mb-3 mb-md-0">
                            <div
                                ...
                                <div>
                                    <div class="font-weight-bold" id="statPaid"
                                        style="font-size:1.4rem;color:#1a1f36;line-height:1;">—</div>
                                    <div style="font-size:.75rem;color:#9ca3af;margin-top:2px;">Paid Holidays</div>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    {{-- ============================================================
     ...
     ============================================================ --}}
    <div class="modal fade" id="holidaysModal" tabindex="-1" role="dialog" aria-hidden="true">
            ...
            <div class="modal-content"
                ...
                <div class="modal-body" style="padding:28px 24px;background:#fff;">
                    ...
                    <div class="form-group mb-4">
                        <label for="is_paid"
                            style="font-weight:600;font-size:.83rem;color:#374151;letter-spacing:.2px;margin-bottom:6px;">
                            IS PAID <span class="text-danger">*</span>
                        </label>
                        ...
                        <span class="text-danger small d-block mt-1" id="err_is_paid"></span>
                    </div>
                    ...
                    <div class="form-group mb-0">
                        ...
                        <select id="status" class="form-control"
                            style="border-radius:10px;border:1.5px solid #e5e7eb;padding:10px 14px;font-size:.9rem;resize:none;transition:border-color .2s;">
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            const BASE_URL = '{{ route('holidays.index') }}';
            const DATA_URL = '{{ url('holidays/data') }}';
            ...
            const table = $('#holidaysTable').DataTable({
                ...
                ajax: {
                    ...
                    dataSrc: function(json) {
                        ...
                        $('#statPaid').text(data.filter(h => h.is_paid).length);
                        return data;
                    },
                    error: function(xhr) {
                ...
                language: {
                    ...
                    emptyTable: '<div style="padding:40px 0;color:#9ca3af;"><i class="fas fa-inbox" style="font-size:2rem;display:block;margin-bottom:10px;opacity:.4;"></i>No holidays found.</div>',
                    info: 'Showing _START_–_END_ of <strong>_TOTAL_</strong> holidays',
                    paginate: {
    ...
    `
        ...
        ` :
                    ...
                    },
                    {
                        ...
                        render: s => s ?
                            '<span class="badge-active" style="background:#d1fae5;color:#059669;"><i class="fas fa-check" style="font-size:.5rem;"></i><span>Paid</span></span>' :
                            '<span class="badge-inactive" style="background:#fef2f2;color:#ef4444;"><i class="fas fa-times" style="font-size:.5rem;"></i><span>Unpaid</span></span>'
                    },
                    {
                        data: 'status',
                        className: 'text-center',
            ...
            /* ── Modal: Add ─────────────────────────────────────────── */
            $('#holidaysModal').on('show.bs.modal', function(e) {
            ...
            $(document).on('click', '.btn-edit', function() {
                ...
                $.get(BASE_URL + '/' + id)
                    ...
                    .fail(function() {
                        toastr.error('Failed to load holiday data.');
                        setTimeout(() => $('#holidaysModal').modal('hide'), 1500);
                    })
                    .always(function() {
            ...
            $(document).on('click', '#btnSave', function() {
                ...
                const payload = {
                    holiday_name: $('#holiday_name').val().trim(),
            ...
            /* ── Modal close: reset ─────────────────────────────────── */
            $('#holidaysModal').on('hidden.bs.modal', resetForm);

            /* ── Helpers ────────────────────────────────────────────── */
            function resetForm() {
                $('#holiday_id, #holiday_name,#description, #is_paid, #start_date, #end_date').val('');
                $('#status').val('');
                clearErrors();
            }

            function clearErrors() {
                ['holiday_name', 'description', 'is_paid', 'status', 'start_date', 'end_date'].forEach(function(f) {
